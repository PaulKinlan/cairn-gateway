...
function exact(value: Record<string, unknown>): boolean {
  return Object.keys(value).filter((key) => value[key] !== undefined).sort().join("\0") ===
    keys.filter((key) => value[key] !== undefined).sort().join("\0");
}
export async function verifyRequestProof(
  proof: DetachedProof,
  publicJwk: JsonWebKey,
  expected: Omit<RequestProofPayload, "issued_at" | "nonce">,
  now: number,
): Promise<boolean> {
  const p = proof.payload;
  if (
    !p || typeof p !== "object" || !exact(p as unknown as Record<string, unknown>) || p.v !== 1 ||
    p.method !== expected.method || p.authority !== expected.authority ||
    p.path !== expected.path || p.query !== "" || p.audience !== "urn:cairn:gateway" ||
    p.body_sha256 !== expected.body_sha256 || p.device_id !== expected.device_id ||
    p.agent_id !== expected.agent_id || p.grant_id !== expected.grant_id ||
