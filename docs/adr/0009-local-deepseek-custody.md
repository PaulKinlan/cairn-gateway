...
thinking disabled, following official documentation retrieved 2026-08-12.
...
sanitized receipt metadata is stored at `~/.local/state/cairn/deepseek.json`. On restart, the
custodian rechecks Secret Service; no key re-entry is required when Secret Service is available. The
random dispatch credential is intentionally regenerated and never persisted.
